...
class Mutator {
  ...
  public:
    ...
    void analyze_scope(IR *stmt_root);                                                                        // done //reset define_library_ first, and fill it
    map<IR *, vector<IR *>> build_graph(IR *stmt_root, map<int, map<DATATYPE, vector<IR *>>> &scope_library); // done //specify case handled here
    ...
    ~Mutator();
    void debug(IR *root, set<string> &names);
