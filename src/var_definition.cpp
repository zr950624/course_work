...
void init_convert_chain(){
    ...
    if(!has_init){
        	v_convert.push_back(make_pair("ANYTYPE", "ALLTYPES")); 
	v_convert.push_back(make_pair("ALLCOMPOUNDTYPE", "ALLTYPES")); 
	v_convert.push_back(make_pair("ALLFUNCTION", "ALLTYPES")); 
...
void init_basic_types(){
    ...
    vector<string> v_basic_types = {"ANYTYPE"};
...
shared_ptr<CompoundType> make_compound_type_by_scope(shared_ptr<Scope> scope, string& structure_name){
    shared_ptr<CompoundType> res;

    res = get_compound_type_by_type_id(get_type_id_by_string(structure_name));
    if(res == nullptr) {
    ...
    if(DBG) cout << "FUCK me in make compound typeid:"<< res->type_id_ <<" Scope id:" << scope->scope_id_ << endl;
